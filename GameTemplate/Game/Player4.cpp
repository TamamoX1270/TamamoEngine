...
void Player4::Update()
{
	...
	if (shine == true) {
		guard = false;
		m_player4.PlayAnimation(enAnimClip_GrabHit, 0.2f);
		m_player4.Update();
		AfterCatch();
		return;
	}
...
void Player4::AnimationState()
{
	...
	if (m_playerState != 3 && m_playerState != 4 && m_playerState != 5 && m_playerState != 6 && m_playerState != 7 && m_playerState != 8 && m_playerState != 9) {
		...
		else if (g_pad[3]->IsTrigger(enButtonX) && moveSpeed.y == 0.0f) {
			//効果音を再生する。
...
void Player4::ManageState()
{
	...
	case 15:
		m_player4.PlayAnimation(enAnimClip_Repel, 0.2f);
		m_isUnderAttack = false;
		m_catch = false;
		m_2 = false;
		m_3 = false;

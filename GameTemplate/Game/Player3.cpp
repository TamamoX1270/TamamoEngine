...
bool Player3::Start()
{
	...
	m_position = Vector3(100.0f, 0.0f, 0.0f);
...
void Player3::Update()
{
	...
	//フォントの位置を設定。
	m_fontRender.SetPosition(Vector3(340.0f, -400.0f, 0.0f));
	//フォントの大きさを設定。
	...
	//リングアウトしたら。
	if (m_out == true) {
		m_player3.PlayAnimation(enAnimClip_RingOut, 0.2f);
	...
	//HPがなくなったら。
		...
		m_player3.PlayAnimation(enAnimClip_Death, 0.2f);
		if (m_player3.IsPlayingAnimation() == false) {
			m_hp = -1;
		}
		m_characterController.RemoveRigidBoby();
	...
	if (shine == true) {
		guard = false;
		m_player3.PlayAnimation(enAnimClip_GrabHit, 0.2f);
		m_player3.Update();
		AfterCatch();
		return;
	}
...
void Player3::Move()
{
	...
	if (g_pad[2]->IsTrigger(enButtonY) && m_soysaucecount >= 1)
	{
		...
		//3Pが醤油弾を撃った。
		m_soysaucebullet->SetSoySauceNewGOPlayer(3);
		//効果音を再生する。
		SoundSource* P3se = NewGO<SoundSource>(10);
		P3se->Init(10);
		P3se->Play(false);
	}
...
void Player3::AnimationState()
{
	...
	if (m_playerState != 3 && m_playerState != 4 && m_playerState != 5 && m_playerState != 6 && m_playerState != 7 && m_playerState != 8 && m_playerState != 9) {
		...
		else if (g_pad[2]->IsTrigger(enButtonX) && moveSpeed.y == 0.0f) {
			//効果音を再生する。
...
void Player3::ManageState()
{
	...
	case 15:
		m_player3.PlayAnimation(enAnimClip_Repel, 0.2f);
		m_isUnderAttack = false;
		m_catch = false;
		m_2 = false;
		m_3 = false;
		...
		if (collision->IsHit(m_characterController))
		{
			...
			/*else {
				if (a.x > 0) {
					...
					//FindGO<Player1>("player1")->SetPlayer1PlayerState15();
				}
			}*/
		}
	}
	...
	for (auto collision : collisions6)
	{
		...
		if (collision->IsHit(m_characterController))
		{
			...
			if (guard != true) {
				...
				SoundSource* P2se = NewGO<SoundSource>(13);
				P2se->Init(13);
				P2se->Play(false);
